...
app.get("/account/:currentEmail", async (req, res) => {
   const currentEmail = req.params.currentEmail;
   console.log('currentEmail',currentEmail)
   const sql = `SELECT u_first, u_last FROM user_info WHERE u_email = "${currentEmail}"`;
...
app.get("/store/:p_id", async (req, res) => {
   ...
   const sql = `
      SELECT product_info.p_id ,product_info.p_img, product_info.p_type, product_info.p_name, product_info.p_price, product_info.p_describe, product_info.p_rating, product_detail.pd_light, pd_temp, pd_water
      FROM product_info
...
app.post("/record-slip", async (req, res) => {
   ...
   const sql = `INSERT INTO slip_info (u_email, p_id) VALUES ("${email}", "${productID}")`;
   const result = await queryDB(sql);
   res.end("Record to database successfully");
});
